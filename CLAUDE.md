...
- **Farb-Regel (wichtig für neue Komponenten):** Untergrenze **`text-paper/55`** auf dunklem, **`text-ink/75`** auf hellem Grund (**seit 01.08. von /60 angehoben** — /60 hält nur mit dem Original-Ink `#0a0807`, nicht mit Julias wärmerem `#2a211b`). Eyebrows in Braun auf hellem Grund: **`text-clay-deep`**, nicht `text-clay`. **`text-pink` (#ff2e88) schafft auf hellem Grund nur 3,02:1** → dort **`text-pink-deep`** verwenden (Original 5,0:1, julia-braun 4,7:1). Auf Ink ist Pink mit 5,7:1 in Ordnung.
...
- **⚠️ Kontrast ist palettenabhängig — das Farb-Panel kann AA brechen.** Messung 01.08. nach den Fixes: **Original 0 Verstöße · „Julia — Dunkelbraun" 5 · „Julia — Hell" 59.** Die 5 sind durchweg *großer Display-Text in Marken-Pink auf hellem Grund* (2,48:1 statt 3:1). „Julia — Hell" invertiert ink/paper und ist so **nicht einsetzbar**. **Vor Go-Live muss feststehen, welche Palette ausgeliefert wird**, und danach neu gemessen werden.
