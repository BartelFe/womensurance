...
- **⚠️ Kontrast ist palettenabhängig — das Farb-Panel kann AA brechen.** Endstand 01.08.: **Original 0 · „Julia — Dunkelbraun" 0** Verstöße auf allen 6 Routen (Julia hat sich für Dunkelbraun entschieden). **„Julia — Hell" bleibt mit 59 Verstößen unbenutzbar** — das Preset invertiert ink/paper, dadurch steht heller Text auf hellem Grund; müsste komplett neu gebaut werden, falls es je gewünscht wird.
- **Dünnste Stelle:** `pink-display` auf `paper` liegt bei **3,01:1** gegen gefordert 3,0 — beide Paletten. Das ist Absicht: Zielwert exakt 3,0, damit das Original-Pink unangetastet bleibt. Wer Reserve will, setzt das Ziel in `deriveTokens` auf 3,2 — dann dunkelt aber auch das Original-Pink nach. Auf `bone` sind es entspannte 3,37–3,44:1.
