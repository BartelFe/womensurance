...
- [ ] **Prüfen, mit welcher Sanity-Gesellschaft der Vertrag tatsächlich besteht.** Die DPA nennt als Datenimporteur „Sanity AS **oder** Sanity US Inc.", je nachdem, wer im Agreement steht. AVV Anlage 3 Nr. 2 und die Datenschutzerklärung nennen beide **Sanity US Inc.** Ist es in Wahrheit die Sanity AS (Oslo, also EWR), stimmt die Drittland-Aussage nicht mehr. Nachsehen in der Rechnungs-/Vertragsansicht unter `manage.sanity.io`. Die Richtung ist ungefährlich (die US-Angabe ist die vorsichtigere), sauber ist es trotzdem nur, wenn es stimmt.
