...
- [x] **Domain: im Termin am 06.08.2026 uebergeben, die DVM-IT stellt die Eintraege.** Vertragsfrage damit erledigt.

  **Live geprueft am 06.08.2026** (Google DoH + DENIC RDAP): Nameserver `ns1043.ui-dns.de`, `ns1114.ui-dns.com`, `ns1018.ui-dns.org`, `ns1099.ui-dns.biz`, SOA-Kontakt `hostmaster.1und1.com` ⇒ **IONOS**. DENIC nennt den Registrar aus Datenschutzgruenden nicht mehr, aber **dvm.de liegt auf exakt derselben Infrastruktur** (gleiche IONOS-Nameserver, gleicher Mailanbieter). Die Domain wurde also aus dem bestehenden IONOS-Konto von DVM angelegt, nicht privat. DENIC: Status `active`, letzte Aenderung 25.06.2025.

  ⚠️⚠️ **Auf womensurance.de laeuft produktiver Mailverkehr.** `MX → mx01–mx04.hornetsecurity.com`, `TXT → v=spf1 redirect=womensurance.de.spf.hornetdmarc.com`, genau wie bei dvm.de. **Bei der Umstellung ausschliesslich den A-Record (Apex) und den www-Eintrag anfassen. MX und TXT bleiben unberuehrt**, sonst steht die Mail. Das war vorher nirgends dokumentiert.

  Weiterhin gilt: Umstellung per A-/CNAME, **nicht** per Nameserver-Delegation, damit DVM die DNS-Hoheit behaelt. Die Werte **aus dem Vercel-Dashboard** nehmen, Vercel vergibt projekteigene CNAME-Ziele (`<hash>.vercel-dns-0xx.com`), das ueberall zitierte `cname.vercel-dns.com` ist ueberholt. **Reihenfolge: erst Vercel Pro, dann Domain.**

  **Nebenfund:** `womensurance.wordpress.com` existiert, ist aber nur eine leere „Demnaechst verfuegbar"-Platzhalterseite mit dem Slogan „Versicherung ist Feminin". Im DNS zeigt nichts darauf, die Domain stand unveraendert auf der IONOS-Parkseite `217.160.0.99`. ⚠️ **Risiko benannt:** Wer dort auf „Domain verbinden" klickt, ueberschreibt genau den A-Record und je nach Weg auch die MX-Eintraege.

- [x] **Tracking-Tags sind raus aus dem Projektumfang (Entscheidung DVM, 06.08.2026).**
  - **LinkedIn Insight Tag: braucht DVM nicht.** Damit erledigt sich auch der offene Punkt aus der Datenschutzerklaerung, ob LinkedIn unter der DPF-Zertifizierung von Microsoft laeuft — der Abschnitt beschreibt einen Dienst, der nicht eingebunden wird.
  - **Meta-Pixel: verschoben.** DVM hat ein Konto, nutzt es aber noch nicht und muss erst intern klaeren, wie sie es datenschutzrechtlich handhaben. Wird nachgezogen, **falls** DVM bereit ist, und haengt nicht am Projektendtermin. Felix ist bei der Klaerung bewusst raus, das ist DVM-Sache.
  - **Folge: kein GTM-Aufbau noetig.** Der Container `GTM-WHJ254XW` bleibt bestehen und gehoert DVM, `VITE_GTM_ID` bleibt in Vercel aber leer ⇒ kein Tracking, der Container laedt gar nicht.
  - ⚠️ **Wenn der Pixel spaeter doch kommt, ist der Schritt aus A.16 zwingend:** Bei jedem Tag unter *Erweiterte Einstellungen → Einwilligungseinstellungen* eine **zusaetzliche Einwilligungspruefung** auf `ad_storage`, `ad_user_data`, `ad_personalization` setzen (GA4: `analytics_storage`). Google-Tags achten selbst auf den Consent Mode, **Meta-Pixel nicht** — ohne diesen Haken feuert er auch bei Ablehnung und die Marketing-Kategorie im Banner waere wirkungslos.
- [x] **Sanity-Region: bewusst abgehakt ohne Nachschlagen (06.08.2026, Entscheidung Felix).** Begruendung, damit es niemand neu aufmacht: **Die Antwort kann die Dokumentation nicht widerlegen, sie ist auf den ungünstigeren Fall geschrieben.** Die Datenschutzerklaerung nennt in Abschnitt 2 ausdruecklich **Sanity US Inc., 351 California St, San Francisco** als Anbieter, behandelt die Uebermittlung also als Drittlandtransfer und stuetzt sie auf die **Standardvertragsklauseln** aus dem DPA (Ziffer 10.1, EU 2021/914 Modul 2, siehe A.8). Laege die Region tatsaechlich in der EU, waere die Erklaerung lediglich konservativer als noetig, aber nie falsch.

  Dazu kommt: Inhalte werden **beim Bauen** geholt (`scripts/fetch-content.mjs`), Bilder laufen ueber den eigenen Proxy `/api/image`. **Es erreichen keine Besucherdaten Sanity**, dort liegen nur Julias redaktionelle Inhalte und ihr Zugang. Das ist der Grund, warum die Region hier kaum Gewicht hat.

  ⚠️ Falls es doch je gebraucht wird: Die Region steht **nur** in `manage.sanity.io` unter Projekt → Settings, ueber den Connector ist sie nicht auslesbar (geprueft 06.08.2026, weder `list_projects` noch `list_datasets` geben sie aus). Projekt-ID `10o1bkel`, Name „DVM - womensurance", Organisation `oUZ0rnVeC`, angelegt 14.07.2026, 2 Mitglieder.

  **Nebenfund beim Nachsehen:** Das Dataset `production` steht auf `aclMode: public`. Das ist fuer diesen Aufbau richtig so, der Build holt die Inhalte ohne Token. Es heisst aber, dass veroeffentlichte Inhalte fuer jeden lesbar sind, der die Projekt-ID kennt. Fuer eine oeffentliche Website ohne Bedeutung, nur nichts Vertrauliches im Studio ablegen.

  **Der DPF-Status von Sanity bleibt bewusst ungeprueft**, weil die Website sich auf die Standardvertragsklauseln stuetzt und nicht auf das Data Privacy Framework. Nur falls jemand spaeter DPF behaupten will, muesste das auf dataprivacyframework.gov nachgesehen werden.
- [x] **Testimonials: kein offener Punkt mehr (06.08.2026).** Felix hat Julia im Termin gezeigt, wie sie Stimmen selbst ueber Sanity eintraegt. Sie pflegt sie ein, sobald sie welche hat. **Nicht Voraussetzung fuer die Abnahme und kein Go-Live-Blocker** — die Stimmen-Sektion bleibt ausgeblendet, solange keine da sind, und der Build meldet das als Hinweis. Damit ist von Julias Seite nichts mehr offen: About-Text final, alle drei Inhaltsbilder mit Alt-Texten.
...
## A.17 · Design-Korrekturen 06.08.2026 (Felix, nach Sichtung auf dem Handy)

**1. Totes Stück zwischen Kopfbereich und Rentenbescheid.** Beim Scrollen stand bis zu eine volle Bildschirmhöhe leer. Ursache: `OpeningStatement` ist `h-[100svh]` hoch, `TheNotice` blendete Überschrift und Karte aber erst in der **gepinnten** Timeline ein, und die startet zwingend bei `start: 'top top'`. Zwischen „Kopfbereich ist raus" und „Pin greift" lag also nichts.

**Gemessen statt vermutet** (375×812, gebauter Stand): bei scrollY 700 endete der letzte sichtbare Inhalt bei 106 px, darunter 706 px auf Deckkraft 0, also **87 % des Bildschirms leer**. Die Deckkraft von `intro` und `card` war bei scrollY 400/600/700/812 durchgehend 0 und sprang erst danach.

**Lösung:** eigener, nicht gepinnter ScrollTrigger (`top 85%` bis `top 20%`, `scrub: 0.5`), der beide während des Hereinscrollens einblendet. Kein Verschieben der Startzeit in der Timeline, weil der Pin bei `top top` beginnen muss. Die Posten starten dadurch bei `rowsStart = 0.25` statt 0.8 (die 0.8 waren der Platz für Überschrift und Karte), alles Weitere hängt an `afterRows` und rutscht mit. Nachher: 300 px → 0,34 · 500 px → 0,72 · 700 px → 1. Die gewünschte Überblendung bleibt, auch beim Zurückscrollen.

**2. Unterlänge des „g" in „Auf Augenhöhe." abgeschnitten.** `.line-mask` hatte Luft nur oben (`padding-top: 0.12em` gegen gekappte Ü-Punkte), unten null. Gemessen: **3,6 px auf dem Handy** (41,6 px Lato Bold Italic) und **9 px auf dem Desktop** (129,6 px). Es war also auf beiden Größen beschnitten, oben nur weniger auffällig.

⚠️⚠️ **Der erste Versuch war falsch, und das ist die eigentliche Lehre.** `padding-bottom` mit ausgleichendem negativem `margin-bottom`, analog zu oben, **veränderte das Layout**: Die Zeilen sind Geschwister, ihre vertikalen Margins fallen zusammen, und beim Zusammenfallen gewinnt nur der negativste Wert. Das Padding beider Zeilen bleibt aber stehen. Ergebnis: Seite 18 px höher, Zeilenabstand in den Überschriften verändert. **Aufgefallen nur, weil die Seitenhöhe vorher und nachher verglichen wurde** (14043 px). Bei jeder Änderung an `.line-mask` also die Seitenhöhe gegenprüfen.

**Lösung:** `clip-path: inset(0 0 -0.16em 0)` **statt** `overflow: hidden`. Zieht nur das Sichtfenster auf und lässt die Box in Ruhe. Beides zusammen ginge nicht, die Flächen würden sich schneiden und die Unterlänge bliebe gekappt. Nachher: Seitenhöhe exakt 14043 px, Zeilenabstände konstant 40,8 px, 3,2 px Luft unter der Tinte auf dem Handy, 11,7 px auf dem Desktop, kein Querscrollbalken. Die Einblendung leckt nicht: Die Zeile startet auf `yPercent: 100`, ihre Tinte beginnt rund 10 px unterhalb der Boxkante und bleibt damit auch im größeren Fenster verdeckt, zusätzlich läuft `opacity` von 0 mit.

⚠️ **Werkzeug-Hinweis:** Der Vorschau-Browser meldete bei der Voreinstellung „desktop" ein Viewport von 0×0 (die Pane rendert nicht), wodurch jede Messung 0 ergab und alles fälschlich als „sichtbar" durchging. Größe explizit setzen (`width/height`) und `innerHeight` vor der Messung prüfen.
