...
- [ ] ⚠️⚠️ **Kennzahlen: Quellen recherchiert und dokumentiert (04.08.2026).** Vollständige Übersicht aller Zahlen der Website mit Fundstellen liegt unter `Kunden\Womensurance (DVM)\Zahlen und Quellen.md` (für Julia gedacht, trennt amtliche Statistik von eigener Beispielrechnung). **Erledigt:** Gender Pay Gap in Sanity von 18 % auf 16 % korrigiert und belegt, Teilzeit und Care-Arbeit belegt (alle drei veröffentlicht). **Offen bleibt allein der Gender Pension Gap**, weil dessen Umstellung Code berührt.

  | Kennzahl | im Text | Destatis aktuell | Fundstelle |
  |---|---|---|---|
  | Gender Pension Gap | 39,4 % | **36,0 %** (2025, eigene Alterseinkünfte) | Tabelle „Alterseinkünfte nach Geschlecht sowie Gender Pension Gap" |
  | Frauen in Teilzeit | 49 % (M: 12 %) | **stimmt** (2024) | PM Nr. 175 vom 19.05.2025 |
  | Gender Pay Gap | ~~18 %~~ → **16 %** erledigt | 16 % (2025, unbereinigt) | PM Nr. 453 vom 16.12.2025 |
  | Gender Care Gap | 44 % | **44,3 %** (ZVE 2022) | PM Nr. 073 vom 28.03.2024 (Korrektur von 43,8 %) |

  **Gender Pay Gap:** 18 % war das Berichtsjahr 2023. Destatis meldete im Februar 2025 den Rückgang auf 16 % (PM Nr. 056) und im Dezember 2025, dass es bei 16 % bleibt. Der Wert ist zwei Berichtsjahre alt.

  **Gender Pension Gap, der heikle Fall:** 39,4 % war nie erfunden, sondern stand so in PM Nr. N016 vom 24.04.2024 für das Berichtsjahr 2023 (ohne Hinterbliebenenrenten). Destatis hat den Wert seither aber **selbst revidiert**: In der laufenden Tabelle steht 2023 heute mit 37,7 %, und der neueste Wert ist 2025 mit **36,0 %**. Die Zahl mit Jahresangabe zu zitieren hilft also nicht, weil auch das Berichtsjahr 2023 inzwischen anders lautet. Die Reihe (ohne Hinterbliebenenrenten): 2020 40,8 · 2021 41,0 · 2022 38,3 · 2023 37,7 · 2024 36,9 · 2025 36,0. Einschließlich Hinterbliebenenrenten 2025: 24,2 %. **Die Definition „ohne Hinterbliebenenrenten" ist für diese Website die richtige**, weil es um selbst erworbene Ansprüche geht.

  ⚠️ **39,4 hängt an sechs Stellen im Code**, nicht nur im CMS: `Loader.jsx` (Zähler im Ladebildschirm), `useGapState.jsx` (`BASE_GAP`), `data/gapStats.js` (Rückfallwert), zwei Kommentare in `OpeningStatement.jsx` zur Chart-Geometrie sowie ein Fließtext in `content/themenseiten.json`. Ein Wechsel auf 36,0 ist deshalb **keine reine CMS-Änderung**. Die Kurvengeometrie ist auf „39,4 % ≙ 150 px" kalibriert, das müsste mitgezogen werden. **Entscheidung liegt bei Felix und Julia.**

- [ ] ⚠️ **`src/data/gapStats.js` ist toter Code, und damit werden die vier Sanity-Kennzahlen nirgends angezeigt.** Nachgewiesen 04.08.2026: Nichts importiert `gapStats` oder `HERO_GAP_VALUE`, und `content/kennzahlen.json` landet nicht im Bundle (der Treffer auf „Gender Pay Gap" im Bundle stammt aus einem FAQ-Text in `themenseiten.json`). **Folge: Julia kann die vier Kennzahlen im Studio bearbeiten, ohne dass sich auf der Seite etwas ändert.** Die tatsächlich sichtbaren Zahlen sind hartcodiert in `useGapState.jsx` (`BASE_GAP`, `BASE_EURO`, `TOGGLE_META`) und `Loader.jsx`. Der Kommentarkopf in `gapStats.js` behauptet ausdrücklich das Gegenteil („`HERO_GAP_VALUE` ist der Ausgangswert des Rechners") und ist damit falsch. **Zu entscheiden:** entweder das Kennzahlen-Band wieder einbauen und den Rechner tatsächlich aus dem CMS speisen, oder `gapStats.js` samt der vier `kennzahl`-Dokumente entfernen. Solange beides koexistiert, ist es eine Falle.
...
✅ **`og:url`, `twitter:url`, `og:image` und `twitter:image` stehen seit 04.08.2026 auf `womensurance.de`** (dazu die Anzeige-Adresse in `public/og-preview.html`). Die Linkvorschau ist damit bis zur DNS-Umstellung kurz kaputt, das war Felix' bewusste Entscheidung, weil die Umstellung unmittelbar bevorsteht. `SITE_URL` in `config/site.js` stand ohnehin schon auf der echten Domain, damit `rel=canonical` von Anfang an aufs Ziel zeigt.
